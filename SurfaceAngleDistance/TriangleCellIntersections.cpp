...
TriangleCellIntersection::TriangleCellIntersection(const std::vector<double> & points, const std::vector<int> & simplices, double box_width, int sub_res, double & average_boxes, double & intersection_per_box){
	...
	for (int n = 0; n < 3; n++){
		lengths[n] = (maxes[n] - mins[n]) / (width * sub_resolution) + 1;
		original_lengths[n] = (maxes[n] - mins[n]) / (width)+1;
	}

	//Preparing triangle intersection vector
	int num_boxes = lengths[0] * lengths[1] * lengths[2];

	int total_box_count = num_boxes;

	try
	{
		list_of_intersecting_simplices = new SubGrid[num_boxes];
		...
		for (int n = 0; n < simplices.size(); n += 3){
			...
			for (int m = 0; m < boxes_intersected.size(); m += 3){
				int a = boxes_intersected[m + 0] / sub_resolution;
				int b = boxes_intersected[m + 1] / sub_resolution;
				int c = boxes_intersected[m + 2] / sub_resolution;

				int a_sub = boxes_intersected[m + 0] % sub_resolution;
				int b_sub = boxes_intersected[m + 1] % sub_resolution;
				int c_sub = boxes_intersected[m + 2] % sub_resolution;

				int sup_index = find_box_index(a, b, c);
				int sub_index = find_box_index_sub(a_sub, b_sub, c_sub);

				if (!list_of_intersecting_simplices[sup_index].non_empty){
					total_box_count += sub_resolution * sub_resolution * sub_resolution - 1;
		...
		intersection_per_box /= total_box_count;
...
void TriangleCellIntersection::free_memory(){
	delete[] list_of_intersecting_simplices;
}

void TriangleCellIntersection::find_boxes_triangle_intersects(const double triangle_points[], std::vector<int> & boxes_intersected){
	...
	if (vector_normal_to_triangle(triangle_points, normal_vector)){
		...
		double adjusted_triangle_points[DIM3 * 3];
		...
		for (int a = lower[0]; a <= upper[0] && a < original_lengths[0]; a++){
			for (int b = lower[1]; b <= upper[1] && b < original_lengths[1]; b++){
				for (int c = lower[2]; c <= upper[2] && c < original_lengths[2]; c++){
					...
					double plane_values_vertices[8];
					for (int x = 0; x <= 1; x++){
						for (int y = 0; y <= 1; y++){
							for (int z = 0; z <= 1; z++){
					...
					if (add_box){
						if (region_test(adjusted_triangle_points, a, b, c)){
							boxes_intersected.push_back(a);
							boxes_intersected.push_back(b);
							boxes_intersected.push_back(c);
						}
					}
				}
			}
...
bool TriangleCellIntersection::region_test(const double triangle[], int a, int b, int c){

	double xyTriangle[6];
	int index = 0;
	for (int n = 0; n < DIM3 * 3 - 1; n++){
		if (n % 3 == 2){
			n++;
		}
...
bool TriangleCellIntersection::triangle_square_intersection(const double triangle2D[], int a, int b){
...
bool TriangleCellIntersection::line_square_intersection(const double line[], const double point[], int a, int b){
...
void TriangleCellIntersection::find_simplices_in_shell_distance_from_cube(const double point[], int shell_level, std::vector<int> & simplices){
	...
	for (int a = max(0, box_coords[0] - shell_level); a < original_lengths[0] && a <= box_coords[0] + shell_level; a++){
		for (int b = max(0, box_coords[1] - shell_level); b < original_lengths[1] && b <= box_coords[1] + shell_level; b++){
			...
			while (c < original_lengths[2] && c <= box_coords[2] + shell_level){
				...
				int current_box_index = find_box_index(a / sub_resolution, b / sub_resolution, c / sub_resolution);

				int current_sub_box_index = find_box_index_sub(a % sub_resolution, b % sub_resolution, c % sub_resolution);
				//Finding all simplices that intersect that box and adds them to the list of simplices

				if (list_of_intersecting_simplices[current_box_index].non_empty){
					for (int n = 0; n < ((std::vector<int>*)list_of_intersecting_simplices[current_box_index].sub_grid)[current_sub_box_index].size(); n++){
						int current_simplex = ((std::vector<int>*)list_of_intersecting_simplices[current_box_index].sub_grid)[current_sub_box_index][n];
...
*/
bool TriangleCellIntersection::vector_normal_to_triangle(const double triangle[], double normal_vector[]){
...
int TriangleCellIntersection::find_box_index_sub(int x, int y, int z){

	int index = x * sub_resolution * sub_resolution + y * sub_resolution + z;
	return index;
